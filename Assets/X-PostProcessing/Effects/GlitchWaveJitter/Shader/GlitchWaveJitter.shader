...
Shader "Hidden/X-PostProcessing/Glitch/WaveJitter"
{
	...
	float4 Frag_Vertical(VaryingsDefault i) : SV_Target
	{
		...
		// -------------------------------Sample RGB Color------------------------------
		float4 colorG = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, float2(i.texcoord.x, uv_y));
		float4 colorRB = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, float2(i.texcoord.x, uv_y + rgbSplit_uv_y));
